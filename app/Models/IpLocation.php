...
class IpLocation
{
    ...
    static function getInstance($ip)
    {
        if (!array_key_exists($ip, self::$instance)) {
            self::$instance[$ip] = new self($ip);
        }
    ...
    public function __construct($ipaddress)
    {
        ...
        foreach ($this->query($ipaddress) as $item) {
            $this->r[] = iconv('gbk', 'utf-8', $item);
        }
    }

    function __toString()
    {
        return implode("", $this->r);
    }
